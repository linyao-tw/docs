 ...
 */
const site = process.env.DOCS_SITE_ORIGIN ?? "https://docs.linyao.tw";
...
export default defineConfig({
	...
	/*
	 * React 只為了用 @linyao.tw/ui 的元件而存在。
	 *
	 * 需要自己管狀態的只有產品切換器與搜尋，兩個都放在同一個 island 裡，
	 * 其餘版面仍然是 .astro 加原生 TS，不會為了一顆按鈕把 runtime 帶進每一頁。
	 */
	integrations: [react()],
	...
	markdown: {
		 ...
		 * 因此明確選用 unified；這個規模的站台在建置時間上感覺不出差別。
		 */
		processor: unified({
