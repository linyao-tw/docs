...
| **媒體輪播** | 一串圖片與影片、每張圖片的停留秒數、填滿方式、背景色   |
...
需要素材的區塊都是按「選擇素材」開一個素材格來挑，不是下拉選單：

- 每個素材是一張大縮圖卡片，**點卡片就選取**，右邊的預覽窗格同時播出那張圖或那支影片，不必靠檔名猜。多選時再點一次就是取消。
- 資料夾排在素材格上方，點進去就換到那一層。
- 可以在資料夾之間切換，也可以搜尋（搜尋跨整個素材庫）。
- 只會列出**處理完成**的素材，挑不到還在轉檔的東西。

### 媒體輪播

輪播是多選的：一次勾很多個，或按資料夾旁邊的「全選」把整個資料夾（含子資料夾）一次加進來。加進來之後可以上下調整順序或移除。

**停留秒數是整份清單共用一個數字。** 實際在排播的人幾乎都讓每一則停一樣久，逐則設定只是把「改成 10 秒」變成要點很多次。

影片不受這個秒數影響，一律**播完整段**才換下一則。
