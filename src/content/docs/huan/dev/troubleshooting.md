...
面向架設與開發的問題。使用者端的問題見[使用教學的疑難排解](/huan/guide/troubleshooting)。
...
反向代理沒有轉發 upgrade 請求。見 [Docker 部署](/huan/dev/deploy/docker#反向代理)的 nginx 範例。
...
repository 還沒在 **Settings → Pages** 把 Source 設成 GitHub Actions。這是一次性的手動步驟，詳見 [GitHub Pages](/huan/dev/deploy/github-pages)。
