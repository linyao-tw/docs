...
| 角色          | 權限                                                         |
| ------------- | ------------------------------------------------------------ |
| `super_admin` | 管理使用者、查看稽核紀錄。不擁有資源，資源路由對它一律回 403 |
| `user`        | 操作自己的素材、版面、排程與裝置，不能管理其他使用者         |

資源以 `owner_id` 歸屬到使用者，每一條查詢都帶擁有者條件，跨使用者的存取一律回 404 而不是 403——回 403 等於承認那筆資料存在。跨擁有者的關聯（把別人的版面指給自己的裝置、把別人的裝置排進自己的排程）另外由資料庫的複合外鍵擋住，不只靠服務層檢查。
