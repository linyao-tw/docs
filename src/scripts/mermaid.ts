 ...
 *
 * 用 ResizeObserver 而不是 window resize：觀察者跟著節點走，節點被客戶端
 * 導覽換掉時它自己就結束了，不會在 window 上留下一串對著舊圖測量的閉包。
 */
 ...
 * 主題觀察者只掛一次。
 *
 * 客戶端導覽會讓每個有圖的頁面都呼叫 renderDiagrams()，每次都新開一個
 * MutationObserver 的話，切一次主題就會同時觸發好幾輪重畫。
 */
...
export function renderDiagrams(): void {
	...
	/** 主題切換時整批重畫。 */
	themeWatcher ??= new MutationObserver(() => void renderAll());
