...
const THEME_KEY = "linyao-docs-theme";
...
/*
 * 站台開著客戶端導覽，initDocs() 每換一頁都會再跑一次。
 *
 * 掛在頁面元素上的監聽器不必管，元素本身已經被換掉了；掛在 document 或
 * window 上的則不會，重複掛的結果是逛到第十頁時一次 resize 觸發十輪，
 * 而且其中九輪測量的是早就從 DOM 移除的舊節點。
 *
 * 因此：window 上永遠只有一個 resize 監聽器，要跟著視窗變動重算的東西
 * 登記到 measureOnResize()，每頁開頭清空一次。
 */
let globalsBound = false;

const onResize: Array<() => void> = [];

function measureOnResize(update: () => void): void {
	update();
...
function setupSidebar(): void {
	...
	measureOnResize(updateMore);

	button.addEventListener("click", () => setOpen(sidebar.dataset.open !== "true"));
	scrim.addEventListener("click", () => setOpen(false));
	if (!globalsBound) {
		// 抽屜每頁都是新的，所以查目前這一頁的那個，不要抓住舊的閉包。
		document.addEventListener("keydown", event => {
			if (event.key !== "Escape") return;
			const open = document.getElementById("sidebar");
			if (open?.dataset.open === "true") open.dispatchEvent(new CustomEvent("sidebar:close"));
		});
	}
	sidebar.addEventListener("sidebar:close", () => setOpen(false));
}
 ...
 * 「複製 Markdown」。
 *
 * 抓的是這一頁的 .md，不是畫面上的 HTML —— 複製渲染後的 DOM 會把表格、
 * 程式碼區塊與提示框全部弄散，貼進聊天視窗的人拿到的是一團爛掉的純文字。
 */
function setupPageActions(): void {
	...
	const source = actions?.dataset.markdown;
	if (!actions || !button || !source) return;

	const label = button.querySelector("span");
	const say = (text: string): void => {
		if (!label) return;
		label.textContent = text;
		setTimeout(() => (label.textContent = "複製 Markdown"), 2000);
	};
...
export function initDocs(): void {
	...
	if (!globalsBound) {
		window.addEventListener(
			"resize",
			() => {
				for (const update of onResize) update();
			},
			{ passive: true }
		);
	}
